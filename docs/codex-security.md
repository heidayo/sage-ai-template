...
SAGE installer 自体の supply chain は GitHub Releases primary distribution + SHA256SUMS で hardening されている ([SPEC-0018](../specs/SPEC-0018-installer-supply-chain-hardening.md))。Codex CI で SAGE を導入する場合は `releases/download/v<X.Y.Z>/install.sh` の tag-pinned URL を使用し、`bash install.sh --verify-checksum --remote` で SHA256 整合性を確認すること。
