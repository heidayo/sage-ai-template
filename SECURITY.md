...
| `installer_url` (auto-update) 取得先の悪意ある書き換え            | [improved (Phase 6.1)] local install-state.yaml の sha256 drift 検出は `--verify-checksum` (TASK-0097)。**GitHub Releases primary distribution + SHA256SUMS 公開 + URL pinning + `--verify-checksum --remote` mode** を [SPEC-0018](specs/SPEC-0018-installer-supply-chain-hardening.md) で実装 (release.yml + scripts/sage-publish.sh 拡張)。Gist は legacy fallback として維持。**cosign keyless signing は SPEC-0019 (Phase 6.2)、SLSA provenance は SPEC-0020 (Phase 6.3)** で扱う |
...
| Lethal Trifecta (private data + untrusted input + exfil vector) の同時成立                | [partial → improved (Phase 2B)] `templates/hooks/lethal-trifecta-detect.sh` で warn-only 検出を実装 (Codex review R3 通り block はしない、人間判断責任は維持) |
...
| Installer maintainability (938-line monolith generate-installer.sh) | [provided] modular structure 7 files in scripts/generator/ ([SPEC-0014](specs/SPEC-0014-installer-modularize.md), Phase 5+, byte-identical refactor) |
