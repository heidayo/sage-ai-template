...
新規 hook test `templates/hooks/tests/test-property-section.sh` を作成 (9 scenarios)、`templates/hooks/tests/run-tests.sh` に統合。SPEC-0015 (NFR-01) と同じく 5 回測定中央値 < 200ms を満たす。

## 入力

- SPEC-0024 FR-07 (9 scenarios の test schema)
...
### templates/hooks/tests/test-property-section.sh (新規、9 scenarios)
...
# 3. SPEC-0024 has >= 5 Properties and every Property has Gate mapping `(Gate N)`
# 4. pilot 3 SPECs have >= 5 Properties and every Property has Gate mapping `(Gate N)`
...
# 8. SKIPPED_WITH_APPROVAL_REQUIRED audit log JSON-lines schema validates
# 9. (backward compat) 既存 SPEC (SPEC-0001..0010) に Property 不在 → WARN-only
...
- [ ] `bash templates/hooks/tests/test-property-section.sh` で 9/9 PASS
- [ ] `bash templates/hooks/tests/run-tests.sh` で全 PASS (既存 189 + 新規 9 = 198+)
...
- [ ] SKIPPED_WITH_APPROVAL_REQUIRED audit log JSON-lines fixture が timestamp / approver / reason / spec_id / property_id を満たすこと
