...
#          - SPEC-0024 itself has Properties (eat-your-own-dog-food)
#          - pilot 3 SPECs (0011 / 0014 / 0015) have ≥5 Properties each
...
# --- Scenario 3: SPEC-0024 itself has Properties (eat-your-own-dog-food) ---
if [[ -f "$SPEC_0024" ]]; then
  prop_count=$(grep -cE "^- \[(INV|PRE|POST|ASM)-[0-9]+\]" "$SPEC_0024" || true)
  if [[ "$prop_count" -ge 5 ]]; then
    echo "ok 3 SPEC-0024 has $prop_count Properties (eat-your-own-dog-food, ≥5)"
    PASS=$((PASS + 1))
  else
    echo "not ok 3 SPEC-0024 has only $prop_count Properties (need ≥5 for platform+Security)"
    FAIL=$((FAIL + 1))
  fi
else
...
# --- Scenario 4: pilot 3 SPECs have ≥5 Properties each ---
pilot_fail=0
pilot_summary=""
for f in "$SPEC_0011" "$SPEC_0014" "$SPEC_0015"; do
  ...
  n=$(grep -cE "^- \[(INV|PRE|POST|ASM)-[0-9]+\]" "$f" || true)
  pilot_summary+=" $base($n)"
  if [[ "$n" -lt 5 ]]; then
    pilot_fail=$((pilot_fail + 1))
  fi
done
if [[ "$pilot_fail" -eq 0 ]]; then
  echo "ok 4 pilot 3 SPECs ≥5 Properties each:${pilot_summary}"
  PASS=$((PASS + 1))
else
  echo "not ok 4 pilot SPECs Property count failed:${pilot_summary}"
  FAIL=$((FAIL + 1))
fi
...
# --- Scenario 8: backward compat — existing SPECs without Properties → WARN-only (NFR-06) ---
...
echo "ok 8 backward compat: $existing_without_props/$existing_total existing SPECs without Properties (WARN-only, NFR-06 incremental migration)"
PASS=$((PASS + 1))
